...
public class FileManager {
    ...
    FileOutputStream os;
    private float ratioForPDR = 4.5f;   // MagneticMeasurementViewWithPDR class 에서 PDR을 이용해서 자기장을 측정할 시 사용되는 hanasquare 맵과 현재 맵의 배율이 4.5배 차이남
                                        // 해당 클래스에서 사용하는 hanasquare 맵의 position 좌표가 (100, 200)일 때 같은 위치를 가르키는 PDR 맵의 좌표는(450, 900)으로 나옴 두 데이터를 비교하기 위해 posX posY에만 배율을 맞춰줌 
        ...
        try {
            ...
            for (int i = 0; i < dataArrayList.size(); i++) {
                String data = dataArrayList.get(i).getX() + "," + dataArrayList.get(i).getY() + "," + dataArrayList.get(i).getZ() + ","
                        + dataArrayList.get(i).getAbs() + "," + (dataArrayList.get(i).getPosX() * ratioForPDR) + "," + (dataArrayList.get(i).getPosY() * ratioForPDR) + "\r\n";
